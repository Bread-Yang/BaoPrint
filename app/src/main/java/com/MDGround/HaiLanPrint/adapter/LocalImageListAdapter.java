...
public class LocalImageListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public LocalImageListAdapter(Context context, int maxSelectNum, int mode, boolean showCamera, boolean enablePreview) {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (getItemViewType(position) == TYPE_CAMERA) {
            ...
            Glide.with(context)
                    .load(new File(image.getImageLocalPath()))
                    .centerCrop()
                    .thumbnail(0.5f)
    ...
    private void changeCheckboxState(ViewHolder contentHolder, LocalMedia image) {
        ...
        if (isChecked) {
            for (LocalMedia media : selectImages) {
                if (media.getImageLocalPath().equals(image.getImageLocalPath())) {
